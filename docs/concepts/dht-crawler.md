...
If you're running behind a VPN that supports port forwarding (Mullvad, AirVPN, Proton, etc.), enabling the forward and matching `BITAGENT_PEER_PORT` to the forwarded port restores peer symmetry. Operator-internal Apollo deployments do this; the public quickstart does not.
